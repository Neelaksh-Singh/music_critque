...
def main():
    st.sidebar.title("🎶 MusicCritique")
    with st.sidebar:
        language = st.selectbox(
            "Language", tuple(map(lambda x: x["name"], SUPPORTED_LANGUAGES))
        )
        ...
        enable_suggestions_for_improvement = st.checkbox(
            "Suggestions for Improvement", value=True
        )
        enable_cover_image_generation = st.checkbox(
...
"""
    ...
    if use_example_file:
        ...
        titles = list(
            map(
                lambda x: f"{x['artist']}-{x['title']} ({x['filename'].split('.')[-1]})",
                examples,
            )
        )
        ...
        file = st.file_uploader(
            label="Please upload your audio file",
            type=[*SUPPORTED_AUDIO_FILE_TYPES, *SUPPORTED_VIDEO_FILE_TYPES],
        )
    ...
    if file is not None:
        ...
        if submit_btn:
            ...
            if not openai_api_key or not aai_api_key:
                st.warning(
                    "Please set your OpenAI and AssemblyAI API keys in the .env file.",
                    icon="⚠",
                )
                ...
                language_code = list(
                    filter(lambda x: x["name"] == language, SUPPORTED_LANGUAGES)
                )[0]["code"]
                ...
                if error:
                    ...
                    polling_endpoint, error = submit_transcription_job(
                        upload_url, aai_api_key, language_code, aai_settings
                    )
                    if error:
                        st.error(error, icon="❌")
                    else:
                        # Poll the endpoint for the transcription results
                        transcription_response, error = get_transcription_results(
                            polling_endpoint, aai_api_key
                        )
                        if error:
                            st.error(error, icon="❌")
                        else:
                            ...
                            if enable_content_moderation:
                                with st.expander("Sensitive Content"):
                                    sensitive_topics = transcription_results[
                                    ...
                                    if sensitive_topics != {}:
                                        ...
                                        sensitive_topics_df = pd.DataFrame(
                                            sensitive_topics.items()
                                        )
                                        sensitive_topics_df.columns = [
                                            "Topic",
                                            "Confidence",
                                        ]
                                        ...
                                        st.success(
                                            "All clear! No sensitive content detected.",
                                            icon="✅",
                                        )
                            ...
                            if enable_topic_detection:
                                with st.expander("Detected Topics"):
                                    st.info(
                                        "The following topics were detected with their respective relevance scores: ",
                                        icon="ℹ",
                                    )
                                    topics = transcription_results[
                                    ...
                                    topics_df["Relevance"] = np.round(
                                        topics_df["Relevance"], 5
                                    )
                                    expanded_topics = (
                                        topics_df["Topic"]
                                        .apply(pd.Series)
                            ...
                            if enable_sentiment_analysis:
                                with st.expander("Sentiment Analysis Results"):
                                    st.info(
                                        "Here are the detected sentiments for parts of your content with their respective confidence scores: ",
                                        icon="ℹ",
                                    )
                                    sentiment_analysis_results = transcription_results[
                            ...
                            if enable_entity_detection:
                                with st.expander("Entity Recognition Results"):
                                    ...
                                    if len(entities) == 0:
                                        ...
                                        st.info(
                                            "Here are the detected entities: ", icon="ℹ"
                                        )
                            ...
                            if enable_criticism or enable_suggestions_for_improvement:
                                ...
                                if enable_criticism:
                                    with st.expander("Criticism Results"):
                                        st.info(
                                            "Here is a critical analysis of your music's content: ",
                                            icon="ℹ",
                                        )
                                        st.markdown(
                                            music_critique_results["critical_analysis"]
                                        )

                                if enable_suggestions_for_improvement:
                                    with st.expander("Suggestions for Improvement"):
                                        st.info(
                                            "Here are some recommendations to improve your music's content: ",
                                            icon="ℹ",
                                        )
                                        st.markdown(
                                            music_critique_results["recommendations"]
                                        )
                            ...
                            if enable_cover_image_generation:
                                with st.expander("Cover Image Suggestion"):
                                    ...
                                    if not openai_api_key:
                                        st.warning(
                                            "OpenAI API Key not set in the .env file.", icon="⚠"
                                        )
                                    else:
                                        ...
                                        st.markdown(
                                            f"Based on the theme: **{music_theme_response}**"
                                        )
                                        st.divider()
                                        st.info(
                                            "Here is a cover image generated for your song",
                                            icon="ℹ",
                                        )
